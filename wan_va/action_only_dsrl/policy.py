...
class LingBotActionOnlyDsrlPolicy(nn.Module):
    """Frozen LingBot backbone with trainable steering actor/critic."""
    ...
    def __init__(self, config):
        ...
        self.dsrl_mode = self.policy_cfg.get("dsrl_mode", "action_only")
        ...
        self.hidden_dims = tuple(self.policy_cfg.get("dsrl_hidden_dims", [128, 128, 128]))
        ...
        self.actor_future_encoder = CompactFutureEncoder(
            input_dim=self.raw_future_dim,
            hidden_dim=self.future_latent_dim,
        ).to(self.device)
        self.steer_actor = GaussianPolicy(
        ...
        self.critic_future_encoder = CompactFutureEncoder(
            input_dim=self.raw_future_dim,
            hidden_dim=self.future_latent_dim,
        ).to(self.device)
    ...
    def freeze_report(self):
        ...
        frozen = _count_params(lingbot_params)
        trainable = _count_params(self.trainable_parameters())
        return {
            "frozen_lingbot_params": frozen,
            "trainable_dsrl_params": trainable,
    ...
    def build_step_batch(self, formatted_obs, future_latents):
        ...
        future_summary = self._summarize_future_latents(future_latents).to(self.device)
        return {
            "images": images,
            "states": states,
    ...
    def act(self, formatted_obs, deterministic=False, use_dsrl=None):
        ...
        if use_dsrl:
            ...
            steer_noise_flat, log_prob = self.steer_actor.sample(
                actor_features,
        ...
        return {
            ...
            "steer_noise_flat": (
                ...
                if steer_noise is not None else torch.zeros(1, self.action_noise_dim, device=self.device)
            ),
...
class ActionOnlyDsrlTrainer:
    def __init__(self, config):
        self.config = config
        self.policy = LingBotActionOnlyDsrlPolicy(config)
    ...
    def _batch_to_dict(self, replay_batch, prefix=""):
        return {
            "images": getattr(replay_batch, f"{prefix}images"),
            "states": getattr(replay_batch, f"{prefix}states"),
            "future_summary": getattr(replay_batch, f"{prefix}future_summary"),
        }
    ...
    def update(self):
        ...
        for _ in range(self.updates_per_step):
            ...
            with torch.no_grad():
                ...
                next_noise_flat, next_log_prob = self.policy.steer_actor.sample(
                    next_actor_features,
                    deterministic=False,
                )
            ...
            sampled_noise_flat, log_prob = self.policy.steer_actor.sample(
                actor_features,
                deterministic=False,
            )
            critic_state_features = self.policy._critic_features(current_batch)
